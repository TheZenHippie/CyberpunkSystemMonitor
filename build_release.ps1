<#
.SYNOPSIS
    Builds, publishes, and packages releases for CyberpunkSystemMonitor for GitHub.
...
Write-Host "  CyberpunkSystemMonitor Release Builder - $tagName" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
dotnet publish "$rootDir\CyberpunkSystemMonitor.csproj" `
    -c Release `
    -r win-x64 `
    --self-contained true `
...
$assets = @("README.md", "LICENSE", "icon.png")
...
$frameworkDepZip = Join-Path $releaseDir "CyberpunkSystemMonitor-$tagName-win-x64-FrameworkDependent.zip"
$standaloneZip = Join-Path $releaseDir "CyberpunkSystemMonitor-$tagName-win-x64-Standalone.zip"
